  ...
  return (
    <>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
        ...
        {/* Layout & Visibility */}
        <SettingsAccordion
          title={t("Disposition et Visibilité (Desktop)")}
          description={t("Organisez l'en-tête de votre Dashboard")}
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
            ...
            <div className="nd-settings-card" style={{ padding: '14px', background: 'rgba(255,255,255,0.02)', border: '1px solid var(--nd-card-border)', borderRadius: 'var(--nd-card-radius)' }}>
              ...
              <p style={{ margin: '4px 0 16px 0', fontSize: '0.7rem', color: 'var(--nd-text-muted)' }}>
                {t("Activez/désactivez les éléments et utilisez les flèches pour définir leur ordre de gauche à droite.")}
              </p>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                {getOrder().map((item, i) => {
                  ...
                  const label = item === 'title' ? t('Titre / Logo') : item === 'search' ? t('Barre de Recherche') : t('Menu (Onglets)');
                  const zoneLabel = i === 0 ? t('Zone Gauche') : i === 1 ? t('Zone Centrale') : t('Zone Droite');
                    ...
                    <div key={item} style={{ display: 'flex', flexDirection: 'column', padding: '14px', background: 'rgba(0,0,0,0.2)', border: '1px solid var(--nd-card-border)', borderRadius: 'var(--nd-card-radius)' }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 12 }}>
                        ...
                        <div style={{ display: 'flex', gap: 6, alignItems: 'center', opacity: !isHidden ? 1 : 0.5, transition: 'opacity 0.2s', flexWrap: 'wrap' }}>
                          ...
                          <button onClick={() => moveItem(i, 1)} disabled={i === 2} style={{ padding: '6px 10px', background: 'var(--nd-bg-alt)', border: '1px solid var(--nd-card-border)', borderRadius: '6px', cursor: i === 2 ? 'not-allowed' : 'pointer', color: i === 2 ? 'var(--nd-text-muted)' : 'var(--nd-text)', opacity: i === 2 ? 0.3 : 1, display: 'flex', alignItems: 'center', gap: 4, fontSize: '0.75rem', fontWeight: 600, whiteSpace: 'nowrap' }} title={t("Descendre")}>
                            {t("Descendre ↓")}
                          </button>
                        </div>
                      </div>
              ...
              <div style={{ marginTop: 16 }}>
                <label style={{ fontSize: '0.7rem', color: 'var(--nd-text-muted)', marginBottom: 4, display: 'block' }}>{t("Mode Split Menu")}</label>
                <p style={{ margin: '0 0 8px 0', fontSize: '0.7rem', color: 'var(--nd-text-dimmed)' }}>
                  {t("Sépare le menu des onglets en deux moitiés pour encadrer l'élément sélectionné.")}
                </p>
                <CustomSelect
                  value={splitMenuAround}
                  onChange={(val: any) => updateSplitMenu(val)}
                  options={[
                    { value: 'none', label: t("Désactivé") },
                    { value: 'title', label: t("Séparer autour du Titre") },
                    { value: 'search', label: t("Séparer autour de la Recherche") },
                  ]}
                />
              </div>

              <div style={{ marginTop: 24, paddingTop: 16, borderTop: '1px solid var(--nd-card-border)' }}>
                <h4 style={{ margin: 0, fontSize: '0.82rem', fontWeight: 600, marginBottom: 8 }}>{t("Apparence du Menu")}</h4>
                <p style={{ margin: '0 0 12px 0', fontSize: '0.7rem', color: 'var(--nd-text-dimmed)' }}>
                  {t("Affichez les icônes des onglets à côté de leur nom dans l'en-tête. Les icônes affichées sont celles configurées dans les paramètres du dock.")}
                </p>
                <div style={{ padding: '14px', background: 'rgba(0,0,0,0.2)', border: '1px solid var(--nd-card-border)', borderRadius: 'var(--nd-card-radius)' }}>
                  <ToggleSwitch
                    ...
                    label={t("Afficher les icônes")}
                    sublabel={config?.settings?.showHeaderMenuIcons ? t("Icônes visibles") : t("Masquées (recommandé)")}
                  />
                </div>
              </div>
        ...
        <SettingsAccordion
          title={t("Titre, Logo & Animations")}
          description={t("Personnalisez le contenu de l'en-tête")}
          icon={<Layers size={18} />}
        ...
        >
          <div style={{ display: 'grid', gap: 10, gridTemplateColumns: '1fr' }}>
            <div>
              <label style={{ fontSize: '0.7rem', color: 'var(--nd-text-muted)', marginBottom: 4, display: 'block' }}>{t("Titre principal")}</label>
              <input type="text" className="nd-input" value={title} onChange={(e) => setTitle(e.target.value)} onBlur={() => updateConfig({ title })} placeholder={t("MON HOME LAB")} style={{ fontSize: '0.75rem', padding: '6px 10px' }} />
            </div>
          </div>

          <div style={{ marginTop: 12 }}>
            <label style={{ fontSize: '0.7rem', color: 'var(--nd-text-muted)', marginBottom: 4, display: 'block' }}>{t("Logo (Remplace le texte si défini)")}</label>
            <div style={{ display: 'flex', gap: 8 }}>
              <input
                type="text"
                className="nd-input"
                placeholder={t("URL ou base64 du logo (SVG, PNG)")}
                value={titleLogo}
            ...
            {demoMode ? (
              <div style={{ marginTop: 8, fontSize: '0.68rem', color: 'var(--nd-text-muted)' }}>{t("Import de logo désactivé dans la démo publique.")}</div>
            ) : (
          ...
          <div style={{ marginTop: 12 }}>
            <label style={{ fontSize: '0.7rem', color: 'var(--nd-text-muted)', marginBottom: 4, display: 'block' }}>{t("Police du titre")}</label>
            <CustomSelect
              value={titleFont}
          ...
          <div style={{ marginTop: 12 }}>
            <label style={{ fontSize: '0.7rem', color: 'var(--nd-text-muted)', marginBottom: 4, display: 'block' }}>{t("Animation du titre")}</label>
            <CustomSelect
