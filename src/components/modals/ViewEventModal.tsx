...
export default function ViewEventModal() {
  ...
  return (
    <div className="nd-modal-overlay" onMouseDown={(e) => { if (e.target === e.currentTarget) setViewEventModal({ open: false }); }}>
      <div className="nd-modal" onClick={e => e.stopPropagation()} style={{ maxWidth: '350px', width: '100%', padding: '24px' }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '20px' }}>
          <h3 style={{ fontSize: '1.1rem', fontWeight: 600, color: 'var(--nd-text)', lineHeight: 1.3, paddingRight: '12px', margin: 0 }}>
            {viewEvent.title}
          </h3>
          <button className="nd-btn" onClick={() => setViewEventModal({ open: false })} style={{ padding: 8 }}>
            <X size={16} />
          </button>
        </div>
